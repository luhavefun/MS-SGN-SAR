...
local_path = {'vgg16': '/media/laomaotao/299D817A2D97AD94/2018/jcli/One-Shot-Ship-Segmentation/snapshots/pretrained/vgg16-397923af.pth'}
...
class VGG(nn.Module):
    ...
    def __init__(self, features, use_decoder=False):
        ...
        self.cls = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=1))
...
def restore(model, pre_model):
    try:
        ...
        for k_model, k_pretrained  in zip(model_dict.keys(), list(pre_model.keys())[:len(model_dict.keys())]):
...
def vgg16(pretrained=False,in_channels=3, **kwargs):
    ...
    if pretrained:
        ...
        pretrain_dict = torch.load(local_path['vgg16'])
        restore(model, pretrain_dict)
    return model
