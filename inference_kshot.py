...
def test(args):
    ...
    recalls = []
    fscores = []
    sar_test_set = SarShipTest(args.img_dir,data_class=args.test_class,mode='test',k_shot=args.k_shot)
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            logits = model.forward_kshot(support_img,support_img_mask,query_img)
            # pred = model.get_pred(logits, query_img)
        ...
        for i in range(args.batch_size):
            tp, tn, fp, fn = measure(pred, query_img_mask)
            iou = tp / (tp + fp + fn)
