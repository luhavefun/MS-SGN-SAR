...
from models.resunet_frn import Encoder,FRN
...
class ResUnetOneShot(nn.Module):
    ...
    def forward_kshot(self,support_img_batch,support_mask_batch,query_img):

        """
        Arguments:
            support_img_batch([B,k_shot*3,C,H,W])
            support_mask_batch([B,k_shot,C,H,W])
            query_img([B,C,H,W])
        """
        out = 0
        k_shot = support_mask_batch.shape[1]
        for i in range(k_shot):
            # print("===support_img_batch shape",support_img_batch.shape)
            # print("===support_mask_batch shape",support_mask_batch[:,i].shape)
            support_img = torch.mul(support_img_batch[:,i*3:(i+1)*3],support_mask_batch[:,i].unsqueeze(1))
            out += self.forward(support_img,query_img)
        return out/5.0

    def get_celoss(self, logits, query_label):
        '''
        logits :[B,2,512,512]
        query_label: [B,1,512,512]
        '''
